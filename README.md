...
One of the problems in the transition to renewable energy systems is the question of how to deal with the time mismatch between energy usage and energy generation. When energy production is high and no one uses it, we waste renewable energy, and when consumption exceeds production, energy is overused. Normally, this imbalance is compensated by energy storage and the use of non-renewable energy sources. <br> UP2US aims to address the problem of unused hours at the community level. An intervention in the public space shows the current energy situation through colours, and a digital interface gives people the information they need to adapt behaviour to the current availability of energy. The communities can independently reduce the pressure on the energy grid during peak hours and thereby contribute to a more balanced energy system.
